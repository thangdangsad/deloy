...
import { Tab, Tabs, Table, Button, Modal, Form, InputGroup, Spinner, Alert, Container, Pagination, Row, Col, Badge } from 'react-bootstrap';
    ...
    const renderTable = (orders, pagination, type) => (
        <>
            <Table striped bordered hover responsive>
                <thead><tr><th>ID</th><th>Khách hàng</th><th>Tổng tiền</th><th>Phương thức TT</th><th>TT Thanh toán</th><th>Trạng thái</th><th>Ngày đặt</th></tr></thead>
                <tbody>
                    {orders.map(o => {
                        ...
                        return (
                            <tr key={`${type}-${id}`} onClick={() => handleViewDetail(type, id)} style={{ cursor: 'pointer' }}>
                                ...
                                <td>
                                    <Badge bg={paymentStatusBadge[o.PaymentStatus] || 'secondary'}>
                                        {o.PaymentStatus === 'Pending' ? 'Chờ TT' : 
                                         o.PaymentStatus === 'Paid' ? 'Đã TT' : 
                                         o.PaymentStatus === 'Failed' ? 'Thất bại' :
                                         o.PaymentStatus === 'Refunded' ? 'Hoàn tiền' : o.PaymentStatus}
                                    </Badge>
                                </td>
                                <td>
                                   <Form.Select
