            ...
            {!cartItems.length ? (
                ...
                <>
                    <Table striped bordered hover responsive>
                        ...
                        <tbody>
                            {cartItems.map((item) => {
                                const outOfStock = item.variant.StockQuantity === 0;
                                const insufficientStock = item.Quantity > item.variant.StockQuantity;
                                return (
                                <tr key={item.CartItemID} className={outOfStock ? 'table-secondary' : ''}>
                                    ...
                                    <td>
                                        <div className="d-flex align-items-center gap-2">
                                            <img src={`${API_BASE_URL}${item.variant.ProductImage}`} alt={item.variant.product.Name} style={{ width: 60, height: 60, objectFit: "cover", opacity: outOfStock ? 0.5 : 1 }} onError={(e) => e.target.src = PLACEHOLDER_IMG} />
                                            <div>
                                                ...
                                                {outOfStock && <div><Badge bg="danger">Hết hàng</Badge></div>}
                                                {!outOfStock && insufficientStock && <div><Badge bg="warning">Chỉ còn {item.variant.StockQuantity}</Badge></div>}
                                            </div>
                                        </div>
                                    </td>
                                    <td style={{ verticalAlign: 'middle' }}>
                                        ...
                                        <Button variant="link" size="sm" className="p-0" onClick={() => { setEditingItem(item); setShowPicker(true); }} disabled={updating || outOfStock}>
